...
Sysrv is a botnet written in Golang (Go), with worm capabilities that drops XMRig crypto miner onto vulnerable hosts (both Linux and Windows). Iterations have taken advantage of a weak passwords and variety of vulnerabilities for different services, including but not limited to: _MySQL, Tomcat, Jenkins, WebLogic and WordPress plugins_. 
...
> I should say it could just be that another actor is utilising the Sysrv binaries for their own ends, similar to other families such as Tsunami.
...
The script checks to see if there is already an instance of kthreadkk running; before requesting the payload if it isn't. Once downloaded, the binary is run with `nohup` and any output suppressed. Clean up is performed after this step of */tmp* directories and the payload binary.
